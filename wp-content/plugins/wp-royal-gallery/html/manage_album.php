...
?>
<?php if(isset($gspl->messages['upload']) && is_array($gspl->messages['upload']) && count($gspl->messages['upload']) > 0): ?>
<div class="updated">
<p class="">
<?php foreach($gspl->messages['upload'] as $msg): ?>
	<span><?php print $msg; ?></span><br/>
<?php endforeach;?>
</p>
</div>
<?php endif; ?>
<script type="text/javascript">var spg_current_album_id = <?php print $album['album_id']; ?>;</script>
<div class="dataTable_wrapper">
	<div class="fg-toolbar ui-toolbar ui-widget-header ui-corner-tl ui-corner-tr ui-helper-clearfix">
		...
		<?php spg_get_table_actions(array('spg_bulk_delete_images' => 'Delete', 
											'spg_bulk_disable_images' => 'Disable', 
											'spg_bulk_enable_images' => 'Enable')); ?>
	</div>
	...
	<tbody>
	<?php require_once SPL_PLUGIN_DIR . '/html/images_rows.php'; ?>
	</tbody>
	</table>
...
<div id="single_upload" class="tab_content">
	<form id="single_upload_form" action="" method="post" enctype="multipart/form-data">
		<input type="hidden" name="task" value="spg_single_image_upload" />
		...
		<tr>
			<td title="Type youtube link or video ID"><?php _e('Youtube Video'); ?></td>
			<td><input type="text" name="yvideo" value="" /></td>
		</tr>
		<tr>
...
-->
	...
	var spg_upload_dir = "<?php  /* add the replace "\\" to work on windows server */  print str_replace("\\","/",spg_get_album_dir($album['album_id'])); ?>";
	var spg_secw = "<?php 
	global $wpdb;
	$query = "SELECT txt FROM {$wpdb->prefix}spg_misc WHERE ione=1 AND itwo=1 AND ithree=1 LIMIT 1";
