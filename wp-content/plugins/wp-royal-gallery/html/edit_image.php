...
if( !($album = $gspl->spg_get_album($album_id)) )
{
	die('Album id does not exists');
}
if( !($image = $gspl->get_image($image_id)) )
{
...
<?php endif;?>
	...
	do_action("spg_admin_head-$hook_suffix");
	do_action('spg_admin_head'); 
	?>
</head>
<body>
<div id="wpwrap">
		...
		<div id="wpbody"><!-- <div id="wpbody-content">  -->
			<div class="wrap" style="padding: 12px;">
				...
				<form id="edit_album_form" action="<?php print admin_url() ?>" method="post" enctype="multipart/form-data">
					...
					<tr>
						<td title="Type youtube link or video ID"><label><?php _e('Youtube Video'); ?></label></td>
						<td><input type="text" id="yvideo" name="yvideo" value="<?php print @$image['yvideo']; ?>" /></td>
					</tr>
					<tr>
						<td><label><?php _e('Image file'); ?></label></td>
						<td>
							<input type="file" name="image_file" value="" />
							<img src="<?php print spg_get_album_url($album_id) . '/thumb/' . $image['thumb']; ?>" />
						</td>
					</tr>
					</table>
					<p>
