...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ spg_upload_dir+"/"+file.name + '&task=spg_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	spg_refresh_images_table(spg_current_album_id);
...
function spg_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=spg_get_albums_table', function(res)
	{
		spg_set_data_tables();
		if (res.status == 'ok') 
		{
...
function spg_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=spg_get_albums_images_table&album_id='+album_id, function(res)
	{
		spg_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'spg_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'spg_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'spg_reorder_image' )
		{
			spg_refresh_images_table(album_id);
		}
		if( task == 'spg_reorder_album' )
			spg_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function spg_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', spg_submit_bulk);
...
function spg_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'spg_bulk_delete_albums' || task == 'spg_bulk_enable_albums' || task == 'spg_bulk_disable_albums')
	{
	...
	else if( task == 'spg_bulk_delete_images' || task == 'spg_bulk_enable_images' || task == 'spg_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = spg_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: spg_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: spg_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: spg_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: spg_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : spg_url+"/js/swfupload/js/swfupload.swf",
				upload_url: spg_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":spg_upload_dir, "secw":spg_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: spg_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.spg_delete_album').live('click', spg_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', spg_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', spg_disable);
